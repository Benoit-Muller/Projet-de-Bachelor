...
\begin{remarque}
As before, a square matrix powered by 0 is the identity matrix. Note that $(A-\l I)^{m-1}\ww\neq0$ implies that in particular $\ww\neq0$. As a consequence, a generalised eigenvector of rank 1 is a usual eigenvector.\com{crossed out in the review but have not seen why}
\end{remarque}
...
\end{theoreme}
\begin{proof}
    Linear algebra result, see \com{reference} for a proof.
\end{proof}
...
We show that this basis is useful, if we use its components as initialisation points.
...
\end{lemme}
\begin{proof}
The invariance of the sets comes from the general form of the eigensolutions presented in \prettyref{th:eigensolutions}. Such a solution looks like $e^{\l t}p(t)$, with an eigenvalue $\l$ and a polynomial $p(t)$ whose coefficients are in the generalised eigenspace of $\l$. It is a linear combination of vectors in the same $\E$ for all $t$. The eigensolutions stay then in the space, as well as the span of them, $\E^\s$.
\\ \\
...
\end{lemme}
\begin{proof}
Linear algebra result, see \com{ref} for a proof.
\end{proof}
